...
class MedlyHold extends Component {
  constructor(props) {
    super(props);
    this.state = {
      videos:[],
  ...
  const data1 = await http.get(apiUrl.apiEnd+"/holdedmedlypost",{ headers: {"Authorization" : `Bearer ${tokenStr}`} })
  ...
  this.setState({videos:data1.data.result,data:data1,selectedVideo:data1.data.result[0],user:user1,flags:data2.data.flagname,topics:data3.data.topicName})
    ...
    handleDeleteVideo= async(dataUpdate,video)=>{
      const tokenStr = localStorage.getItem("token")
        const msg = await http.post(apiUrl.apiEnd+"/adminapprovemedlypost", dataUpdate, { headers: { "Authorization": `Bearer ${tokenStr}` } })
        const user1=auth.getCurrentUser();
    ...
    this.setState({videos:sts,selectedVideo:sts[0],flagData:[]})
      if(this.state.videos.length<5 && this.state.count===0){
        // const allocateddaaata =  http.get(apiUrl.apiEnd+"/moderatorallocate",{ headers: {"Authorization" : `Bearer ${tokenStr}`} })
        const data1 = await http.get(apiUrl.apiEnd+"/holdedmedlypost",{ headers: {"Authorization" : `Bearer ${tokenStr}`} })
